...
class DslProcessor(
    ...
    private fun generate(spec: KSClassDeclaration, resolver: Resolver): Boolean {
        ...
        for (member in hierarchy.abstractFunctions(spec)) {
        ...
        val resultProperties = requiredProperties.map { it.name to it.typeName } +
                ...
                listProperties.map { it.name to "List<${it.elementTypeName}>" } +
                childScopes.map { it.name to it.child.resultQualifiedName }
        if (supertypeDeclaration != null) {
            supertypeInternal = effectiveVisibility(supertypeDeclaration, checker) == "internal"
            ...
            for (member in hierarchy.abstractProperties(supertypeDeclaration, environment)) {
                val property = member.declaration
                val name = property.simpleName.asString()
                val rendered = checker.renderTypeReference(property, property.type, member.environment) ?: break
        ...
        if (!checker.valid) return handled(checker)
        val resultVisibility = if (visibility == "internal" || supertypeInternal) "internal" else "public"

        val code = buildString {
            appendLine("// Generated by dsl-utilities-ksp from $specQualifiedName. Do not edit.")
            if (packageName.isNotEmpty()) {
                appendLine("package $packageName")
                appendLine()
            }

            append("$visibility class ${names.builderName}(")
            requiredProperties.joinTo(this, ", ") { "public override val ${it.name}: ${it.typeName}" }
            appendLine(") : $specQualifiedName {")
            ...
            appendLine("    $resultVisibility fun build(): ${names.resultName} {")
            ...
            for (property in listProperties) {
                for (child in property.children) {
                    append("$visibility inline fun $specQualifiedName.${child.functionName}(")
                    ...
                    if (child.required.isEmpty()) {
                        appendLine()
                        appendLine("$visibility inline val $specQualifiedName.${child.functionName}: Unit")
            ...
            append("$resultVisibility $resultClassKeyword ${names.resultName}(")
            append(
                resultProperties.joinToString(", ") { (name, type) ->
                    "public ${if (name in supertypeOverrides) "override " else ""}val $name: $type"
                }
            )
            append(")")
            ...
            if (names.generateFunction) {
                if (requiredProperties.isEmpty()) {
                    appendLine("$resultVisibility inline fun ${names.functionName}(block: $specQualifiedName.() -> Unit = {}): ${names.resultName} {")
                    appendLine("    val builder = ${names.builderName}()")
                    appendLine("    builder.block()")
                    appendLine("    return builder.build()")
                    appendLine("}")
                } else {
                    appendLine("$resultVisibility inline fun ${names.functionName}(")
                    for (property in requiredProperties) {
                        appendLine("    ${property.name}: ${property.typeName},")
                    }
                    appendLine("    block: $specQualifiedName.() -> Unit = {},")
    ...
    private fun requiredProperty(
        ...
        val typeName = checker.renderTypeReference(property, property.type, environment) ?: return null
    ...
    private fun listProperty(
        ...
        val elementTypeReference = type.arguments.single().type ?: run {
    ...
    private fun childScope(
        ...
        for ((parameter, required) in declaredParameters.zip(child.required)) {
            ...
            val parameterTypeName = checker.renderTypeReference(function, parameter.type, environment) ?: return null
    ...
    private fun resolveChild(declaration: KSClassDeclaration, checker: Checker): ChildSpec? {
        ...
        for (member in Hierarchy(checker).abstractProperties(declaration)) {
            ...
            val typeName = checker.renderTypeReference(property, property.type, member.environment) ?: return null
     ...
     */
    private fun effectiveVisibility(spec: KSDeclaration, checker: Checker): String? {
        var internal = false
        ...
        return if (internal) "internal" else "public"
    }
    ...
    private class Hierarchy(private val checker: Checker) {
        fun abstractProperties(
            ...
            walk(declaration, initialEnvironment) { member, environment ->
                ...
                if (name in members) return@walk
        ...
        fun abstractFunctions(declaration: KSClassDeclaration): List<SubstitutedMember<KSFunctionDeclaration>> {
            ...
            walk(declaration, emptyMap()) { member, environment ->
                if (member !is KSFunctionDeclaration || !member.isAbstract) return@walk
                val name = member.simpleName.asString()
    ...
    private class Checker(private val resolver: Resolver, private val logger: KSPLogger) {
        ...
        fun substituteType(type: KSType, environment: Map<KSTypeParameter, KSType>): KSType {
            ...
            (type.declaration as? KSTypeParameter)?.let { return environment[it] ?: type }
         ...
         * type annotations are preserved. Type parameters of members inherited
         * from generic bases are resolved with [environment] before rendering.
         */
        fun renderTypeReference(
            symbol: KSNode,
            reference: KSTypeReference,
            environment: Map<KSTypeParameter, KSType> = emptyMap(),
        ): String? {
            ...
            if (element is KSCallableReference) {
                ...
                val receiver = element.receiverType?.let { renderTypeReference(symbol, it, environment) }
                val parameters = element.functionParameters.map { parameter ->
                    val typeName = renderTypeReference(symbol, parameter.type, environment) ?: return null
                    val name = parameterName(parameter)
                    if (name == null) typeName else "$name: $typeName"
                }
                val returnType = renderTypeReference(symbol, element.returnType, environment) ?: return null
            ...
            return renderType(symbol, substituteType(reference.resolve(), environment))
        }
